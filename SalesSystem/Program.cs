...
using SalesSystem.Payments;
using SalesSystem.ShoppingCart;
...
while (true)
{
    ...
    Console.WriteLine("Choose 1 for Visa 2 for PayPal :");

    int paymentMethod = int.Parse(Console.ReadLine());

    PaymentFactory processor = paymentMethod == 1 ? new VisaPaymentFactory() : new PayPalPaymentFactory();

    cart.Checkout(customer ,processor);
